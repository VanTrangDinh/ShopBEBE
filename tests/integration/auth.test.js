...
describe('Auth routes', () => {
  describe('POST /v1/api/auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      const res = await request(app).post('/v1/api/auth/register').send(newUser).expect(httpStatus.CREATED);
    ...
    test('should return 400 error if email is invalid', async () => {
      ...
      await request(app).post('/v1/api/auth/register').send(newUser).expect(httpStatus.BAD_REQUEST);
    });
  ...
  describe('POST /v1/api/auth/login', () => {
    test('should return 200 and login user if email and password match', async () => {
      ...
      const res = await request(app).post('/v1/api/auth/login').send(loginCredentials).expect(httpStatus.OK);
    ...
    test('should return 401 error if there are no users with that email', async () => {
      ...
      const res = await request(app).post('/v1/api/auth/login').send(loginCredentials).expect(httpStatus.UNAUTHORIZED);
  ...
  describe('POST /v1/api/auth/logout', () => {
    ...
    test('should return 404 error if refresh token is not found in the database', async () => {
      ...
      await request(app).post('/v1/api/auth/logout').send({ refreshToken }).expect(httpStatus.NOT_FOUND);
    });
  ...
  describe('POST /v1/api/auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      const res = await request(app).post('/v1/api/auth/refresh-tokens').send({ refreshToken }).expect(httpStatus.OK);
    ...
    test('should return 400 error if refresh token is missing from request body', async () => {
      await request(app).post('/v1/api/auth/refresh-tokens').send().expect(httpStatus.BAD_REQUEST);
    });
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      ...
      await request(app).post('/v1/api/auth/refresh-tokens').send({ refreshToken }).expect(httpStatus.UNAUTHORIZED);
    });
  ...
  describe('POST /v1/api/auth/forgot-password', () => {
    beforeEach(() => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      await request(app).post('/v1/api/auth/forgot-password').send({ email: userOne.email }).expect(httpStatus.NO_CONTENT);
    ...
    test('should return 400 if email is missing', async () => {
      ...
      await request(app).post('/v1/api/auth/forgot-password').send().expect(httpStatus.BAD_REQUEST);
    });

    test('should return 404 if email does not belong to any user', async () => {
      await request(app).post('/v1/api/auth/forgot-password').send({ email: userOne.email }).expect(httpStatus.NOT_FOUND);
    });
  });
  ...
  describe('POST /v1/api/auth/reset-password', () => {
    ...
    test('should return 400 if reset password token is missing', async () => {
      ...
      await request(app).post('/v1/api/auth/reset-password').send({ password: 'password2' }).expect(httpStatus.BAD_REQUEST);
    });
  ...
  describe('POST /v1/api/auth/send-verification-email', () => {
    beforeEach(() => {
      jest.spyOn(emailService.transport, 'sendMail').mockResolvedValue();
    });
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).post('/v1/api/auth/send-verification-email').send().expect(httpStatus.UNAUTHORIZED);
    });
  });
  ...
  describe('POST /v1/api/auth/verify-email', () => {
    ...
    test('should return 400 if verify email token is missing', async () => {
      ...
      await request(app).post('/v1/api/auth/verify-email').send().expect(httpStatus.BAD_REQUEST);
    });
